...
batch_size = 10
momentum = 0.9
lambda_variable = 0.00001

# default hyperparameters

learning_rate = 0.8
class_error_threshold = 0.25
...
points = mnist.train.images[:training_size,:]
pointsA = mnist.train.labels[:training_size,:]
...
loss = (tf.reduce_mean(tf.square(tf.subtract(y, decoded))))

if regularization_flag:
    train_op = tf.train.MomentumOptimizer(learning_rate,momentum).minimize(loss+lambda_variable*tf.reduce_sum(tf.square(weightsInHid))+lambda_variable*tf.reduce_sum(tf.square(weightsHidOut)))
else:
...
#checks classification error
def isError(d, outs):
    for i in range(len(d)):
        if(abs(d[i] - outs[i]) > class_error_threshold):
            return True
    return False
...
def checkErrors(ins,outs,flag=False):
    ...
    for k in range(len(ins)):
        ...
        #if (l>0.01): #check this number for part 2
        if(isError(d[0], outs[k])):
            errors = errors +1
        if flag:
